...
public class Envi {
	...
	private static String temp = "AndroidManifest.xml";
	...
	public static int scanDex(BufferedInputStream is, int cr7) {
		...
		try {
			...
			int adler32 = ((0xFF & buffer[11]) << 24) | ((0xFF & buffer[10]) << 16) | ((0xFF & buffer[9]) << 8)
					| (0xFF & buffer[8]);
			int l = SQLiteDatabaseHelper.isInDNA(cr7, adler32);
	...
	public static int scanELF(BufferedInputStream is, int cr7) {
		...
		try {
			...
			crc1.update(buffer);
			int lemp = SQLiteDatabaseHelper.isInDNA(cr7, (int) crc1.getValue());
			if (lemp != -1) {
				tRCode = lemp;
			} else {
				boolean cont = false;
	...
	public static void i(Context cx) {
		...
		try {
			...
			SQLiteDatabaseHelper = new SQLiteDatabaseHelper(cx);
